...
class Settings(BaseSettings):
    ...
    BACKEND_CORS_ORIGINS: List[str] = Field(default_factory=lambda: list(_DEFAULT_CORS_ORIGINS))
    ...
    def apply_environment_defaults(self) -> "Settings":
        ...
        if not self.MONGODB_URI:
            if is_production:
                raise ValueError("MONGO_URL (or MONGODB_URI) is required when ENV=production")
