...
function App() {
  ...
  const isSuperAdminInvite =
    location.pathname === `/${ROUTE_PATHS.INVITE}` && user?.role === ROLES.SUPER_ADMIN;

  return (
    <div className="flex min-h-screen bg-[var(--color-surface-bg)] text-[var(--color-text-primary)] font-['Outfit']">
      <ScrollToTop />
      {(showOrgLayout || isSuperAdminInvite) && <Sidebar />}

      <div className="flex flex-col flex-1 w-full overflow-hidden">
        {(showOrgLayout || isSuperAdminInvite) && <Header />}
