...
const Sidebar: React.FC = () => {
  ...
  return (
    <aside className="z-50 flex h-screen w-72 max-w-[18rem] min-w-[18rem] shrink-0 flex-col border-r border-[var(--color-sidebar-border)] bg-[var(--color-sidebar-bg)] text-[var(--color-sidebar-text)] shadow-2xl">
      ...
      <nav className="custom-scrollbar flex min-h-0 flex-1 flex-col gap-2 overflow-x-hidden overflow-y-auto px-4 py-6">
        {navItems.map((item) => {
          if (item.children?.length) {
            ...
            return (
              <div key={item.name} className="flex min-w-0 flex-col gap-0.5">
                ...
                {isOpen && (
                  <div className="ml-4 flex min-w-0 flex-col gap-1 border-l border-[var(--color-sidebar-border)] pt-1 pl-3">
                    {item.children.map((child) => (
                      <NavLink
                        ...
                        className={({ isActive }) =>
                          cn(
                            'block min-w-0 truncate rounded-xl px-3 py-2.5 text-sm font-medium whitespace-nowrap transition-colors duration-200',
                            isActive
